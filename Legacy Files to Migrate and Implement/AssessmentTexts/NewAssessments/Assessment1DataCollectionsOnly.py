...
WHERE InquiryID IN (
    Select ID
    FROM tblInquiry
    WHERE FranchiesId IN (1,2,3,6,11,15,16,19,24,20,57,60,87)
)
AND tblAssessments.IsDeleted = 0
AND tblAssessments.Time IS NOT NULL
AND tblAssessments. TIME != '00:00:00.0000000'
AND tblAssessments.Date >= DATEADD(HOUR, -1, CAST(CAST(GETDATE() AS DATE) AS DATETIME))
ORDER BY 
	tblAssessments.Date DESC,
	tblAssessments.Time DESC;
""")
