"""
Assessment1SQLiteFunctions.py
Purpose: Provide CRUD operations on the 'AssessmentCache' table in a local SQLite DB,
...
def select_assessment_cache(engine):
    """
    Returns all rows from the 'AssessmentCache' table as a DataFrame.
    """
    query = text("SELECT * FROM AssessmentCache;")
...
def delete_assessment_cache(engine, inquiry_ids_to_delete):
    """
    Deletes rows from 'AssessmentCache' where InquiryID is in inquiry_ids_to_delete.
...
def insert_assessment_cache(engine, new_rows_df):
    """
    Inserts new rows into 'AssessmentCache'. The DataFrame columns should match the table's columns.
    ...
    try:
        ...
        new_rows_df.to_sql('AssessmentCache', con=engine, if_exists='append', index=False)
        session.commit()
    except SQLAlchemyError as e:
        session.rollback()
...
def update_assessment_cache(engine, inquiry_ids_to_update):
    """
    Sets IsText = 'Yes' for the specified InquiryIDs in 'AssessmentCache'.
    This loops one-by-one to avoid SQLite parameter binding issues with IN.
    """
...
if __name__ == "__main__":
    # Quick test
    df_cache = select_assessment_cache(engine)
    print("AssessmentCache contents:\n", df_cache)
