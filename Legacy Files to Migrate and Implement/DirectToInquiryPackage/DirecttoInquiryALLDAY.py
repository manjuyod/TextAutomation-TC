...
franchise_info = [
    (1, 'Test', 'https://tutoringclub.com/test/', 'Daniel', 'bmillares@tutoringclub.com'),
    (57, 'Gilbert', 'https://tutoringclub.com/gilbertaz/', 'Ryan', 'gilbertaz@tutoringclub.com'),
    (24, 'North Fresno', 'https://tutoringclub.com/gilbertaz/', 'Ryan', 'fresnoca@tutoringclub.com'),
    (19, 'Tutoring Club of Tustin', 'https://tutoringclub.com/tustinca/', 'Tim', 'tustinca@tutoringclub.com'),
    (8,'Fountain Valley CA', 'https://tutoringclub.com/fountain-valley-ca/', 'Huy', 'fountainvalleyca@tutoringclub.com'),
    (20,'Clovis', 'https://tutoringclub.com/clovisca/', 'Katie', 'clovisca@tutoringclub.com'),
    (87,'Downey','https://tutoringclub.com/downeyca/','David', 'downeyca@tutoringclub.com'),
...
# ORDER: FranchiseID, FranchiseName, WebString, Director
...
def get_timezone(franchise_id):
    # Nevada centers use Pacific time; Gilbert (57) uses Mountain
...
def is_dst(dt, timezone_str):
    ...
    # Works for both Pacific and Mountain US zones.
...
def main():
    ...
    for msg_id, msg in emails:
        try:
            ...
            ALLOWED_FIDS_FOR_AUTOREAD = {6, 11, 15, 16, 60}
            if not fid_to:
                continue

            sent_utc = extract_sent_utc(msg)
            local_sent = localize_to_franchise(sent_utc, fid_to)
            if fid_to in ALLOWED_FIDS_FOR_AUTOREAD:
                if sent_in_business_window(local_sent):
                    # Within hours (NV): auto-mark read, do not text (humans handle)
                    try:
                        send_telegram_message(
                            f"✉️ Auto-marked as read (within hours) — FID {fid_to}, local sent: {local_sent}",
                            LOG_BOT, LOG_CHAT
                        )
                    except Exception:
                        pass
                    ...
                    # After-hours (NV): let AFTERHOURS script own it → skip in ALLDAY
                    try:
                        send_telegram_message(
                            f"🌙 Skipping after-hours in ALLDAY — FID {fid_to}, local sent: {local_sent}",
                            LOG_BOT, LOG_CHAT
                        )
                    except Exception:
                        pass
